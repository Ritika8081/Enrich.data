...
import { composeOutreach, type OutreachDraft } from "./src/outreach/compose.js";
import { buildHtmlPreview } from "./src/outreach/html_preview.js";
...
function parseLeadLookupArgs(args: string): LeadLookupArgs | { error: string } {
  ...
  const briefMatch = input.match(/--brief\s+"([^"]+)"|--brief\s+'([^']+)'/i);
  ...
  const brief = (briefMatch?.[1] ?? briefMatch?.[2] ?? "").trim();

  if (!name) {
    return {
      error:
        'Missing --name. Usage: /leadfind --name "Full Name" [--email you@example.com] [--phone 123456789] [--brief "..."]\n' +
        "   or: /leadfind Full Name | you@example.com | 123456789",
    };
  }
    ...
    safeParse(value: unknown) {
      ...
      for (const field of [
        ...
        if (config[field] !== undefined && typeof config[field] !== "string") {
          return {
            success: false,
            error: { issues: [{ path: [field], message: `${field} must be a string` }] },
          };
        }
      }
    ...
    jsonSchema: {
      ...
      properties: {
        ...
        outreachApiKey: {
          type: "string",
          description:
            "API key for the outreach LLM (any OpenAI-compatible provider). Free option: Groq (https://console.groq.com). Falls back to GROQ_API_KEY / OUTREACH_API_KEY env.",
        },
        outreachBaseUrl: {
          type: "string",
          description:
            "Base URL for the outreach LLM provider. Defaults to https://api.groq.com/openai/v1. Set to https://api.openai.com/v1 for OpenAI, or http://localhost:11434/v1 for Ollama.",
        },
        outreachModel: {
          type: "string",
          description:
            "Model name for outreach generation. Defaults to llama-3.3-70b-versatile (free on Groq).",
        },
        defaultBrief: {
          type: "string",
          description:
            "Default campaign brief prepended to every outreach generation. Override per-call with --brief \"...\".",
        },
      },
    },
  };
...
const plugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    const outreachApiKey =
      config.outreachApiKey ?? process.env.GROQ_API_KEY ?? process.env.OUTREACH_API_KEY;
    const outreachBaseUrl = config.outreachBaseUrl ?? process.env.OUTREACH_BASE_URL;
    const outreachModel = config.outreachModel ?? process.env.OUTREACH_MODEL;
    const defaultBrief = config.defaultBrief ?? process.env.OUTREACH_BRIEF;
    ...
    const tryComposeOutreach = async (
      lead: { name: string; email?: string; phone?: string },
      enrichment: Parameters<typeof composeOutreach>[0]["enrichment"],
      ...
      handler: async (ctx) => {
          ...
          try {
            ...
            for (const lead of leads) {
              try {
                ...
                if (outreachApiKey && !rows[rows.length - 1].error) {
                  const outreach = await tryComposeOutreach(
                    {
                      name: lead.name,
                      ...(lead.email ? { email: lead.email } : {}),
                      ...(lead.phone ? { phone: lead.phone } : {}),
                    },
                    {
                      ...
                      linkedinInsights: result.linkedinInsights,
                  ...
                  if (outreach.draft) {
                    lastRow.outreachSubject = outreach.draft.subject;
                    lastRow.outreachEmail = outreach.draft.email;
                    lastRow.outreachLinkedinDm = outreach.draft.linkedinDm;
                  } else if (outreach.error) {
                    lastRow.outreachError = outreach.error;
                  }
                }
              } catch (error) {
            ...
            const htmlPreviewPath = outputPath.replace(/\.csv$/i, ".html");
            try {
              const html = buildHtmlPreview(rows, {
                totalRows: rows.length,
                successRows: successCount,
                failedRows: failedCount,
                brief: defaultBrief,
                model: outreachApiKey ? outreachModel ?? "llama-3.3-70b-versatile" : undefined,
              });
              await writeFile(htmlPreviewPath, html, "utf8");
            } catch (previewError) {
              api.logger.warn(
                `[lead-enrichment] failed to write HTML preview: ${
        ...
        try {
          ...
          if (outreachApiKey) {
            ...
            lines.push("");
            lines.push("**AI Outreach Draft**");
            if (outreach.draft) {
              lines.push(`Subject: ${outreach.draft.subject}`);
              lines.push("");
              lines.push("Email:");
              lines.push(outreach.draft.email);
              lines.push("");
              lines.push("LinkedIn DM:");
              lines.push(outreach.draft.linkedinDm);
            } else if (outreach.error) {
              lines.push(`(skipped — ${outreach.error})`);
            }
          }
